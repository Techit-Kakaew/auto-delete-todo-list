...
const Home = () => {
  ...
  const [dataList, setDataList] = useState<MappedData[]>(
  ...
  const getDataListByType = useCallback(
    (dataType: string) => {
  ...
  const handleButtonClick = (currentData: MappedData) => {
    const moveToLast = (prevDataList: MappedData[]) => {
      ...
      const currentIndex = prevDataList.findIndex(
  ...
  return (
    <>
      <Button onClick={() => router.push("/line-message")}>
        Go to line message page
      </Button>
      <Row
        gutter={24}
        style={{
